...
**Single topic:**
1. Run: `python3 /root/docs/swarm/experts.py researcher "<topic>"`
2. Capture the printed task UUID
3. Run: `python3 /root/docs/swarm/wait.py <task_id>`
4. Present the researcher's findings to the user

**Multiple topics (parallel):**
1. For each topic, run `python3 /root/docs/swarm/experts.py researcher "<topic>"` — capture all task UUIDs
2. Run: `python3 /root/docs/swarm/wait.py <id1> <id2> <id3>` (waits for all via LISTEN/NOTIFY)
